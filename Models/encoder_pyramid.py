...
from utils import frame_stacking
...
class Encoder(nn.Module):
    def __init__(self):
        ...
        self.bi_lstm_stack1 = nn.LSTM(input_size=input_size, hidden_size=hp.num_hidden_nodes, num_layers=hp.num_encoder_layer, \
                batch_first=True, dropout=0, bidirectional=True)
        self.stack1 = nn.Linear(hp.num_hidden_nodes*4,hp.num_hidden_nodes*4)

        self.bi_lstm_stack2 = nn.LSTM(input_size=hp.num_hidden_nodes*4, hidden_size=hp.num_hidden_nodes, num_layers=hp.num_encoder_layer, \
                batch_first=True, dropout=0, bidirectional=True)
        self.stack2 = nn.Linear(hp.num_hidden_nodes*4,hp.num_hidden_nodes*4)

        self.bi_lstm_stack3 = nn.LSTM(input_size=hp.num_hidden_nodes*4, hidden_size=hp.num_hidden_nodes, num_layers=hp.num_encoder_layer, \
                batch_first=True, dropout=0, bidirectional=True)
        self.stack3 = nn.Linear(hp.num_hidden_nodes*4,hp.num_hidden_nodes*4)


    def forward(self, x, lengths):
        total_length = x.size(1)
        x = nn.utils.rnn.pack_padded_sequence(x, lengths, batch_first=True)
        self.bi_lstm_stack1.flatten_parameters()
        self.bi_lstm_stack2.flatten_parameters()
        self.bi_lstm_stack3.flatten_parameters()

        x, _ = self.bi_lstm_stack1(x)
        x, lengths = nn.utils.rnn.pad_packed_sequence(x, batch_first=True, total_length=total_length)
        print(lengths)
        batch_size = x.size(0)
        feature_dim = x.size(2)
        total_length = x.size(1)
        if x.size(1)%2:
            x = x[:,0:total_length-1,:]
        x = x.contiguous().view(batch_size, int(total_length//2), feature_dim*2)
        x = torch.tanh(self.stack1(x))

        x, _ = self.bi_lstm_stack2(x)
        batch_size = x.size(0)
        feature_dim = x.size(2)
        total_length = x.size(1)
        if x.size(1)%2:
            x = x[:,0:total_length-1,:]
        x = x.contiguous().view(batch_size, int(total_length//2), feature_dim*2)
        x = torch.tanh(self.stack2(x))

        x, _ = self.bi_lstm_stack3(x)
        batch_size = x.size(0)
        feature_dim = x.size(2)
        total_length = x.size(1)
        if x.size(1)%2:
            x = x[:,0:total_length-1,:]
        x = x.contiguous().view(batch_size, int(total_length//2), feature_dim*2)
        hbatch = torch.tanh(self.stack3(x))
        lengths = lengths//8
        print(lengths)

        return hbatch, lengths
